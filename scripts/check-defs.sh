...
echo "==> the files on disk match the generator"
python3 scripts/gen-kalam.py --check
...
# fmt is scoped to the AUTHORED files. channels/ and workflows/ are generated by the generator
# above and are gitignored -- there is no diff for an untracked file, so there is nothing for a
# house style to keep readable there. What a reviewer reads is connectors/, shared/ and the
# generator itself.
echo "==> fmt"
orion-server fmt --check connectors shared
