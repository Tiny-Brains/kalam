...
- **Capacity is `RUNNER_CRON_WORKERS`.** That many slots on the one match channel, up to Orion's 64 (the
  package's committed `slots` is only a default, and the entrypoint overwrites it), and Orion's pool is one worker larger so the roster clock always has one: with a shared
